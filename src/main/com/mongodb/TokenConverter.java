...
public  class TokenConverter { 
    ...
    public void transformAttrs(DBObject o, boolean toToken) {
        if (o==null)
            return;
        ...
        while (it.hasNext()) {
            ...
            if(entry.getKey().equals("_id")
                    ||entry.getKey().equals(TOKENIZE))
                     continue;
            
            Object value = o.removeField(entry.getKey());
        
            if(toToken)
                o.put(mapFieldToToken(entry.getKey()), value);
            ...
            if(value instanceof List || o instanceof BasicBSONList) {
                for(Object dbo : (List)value)
                    if(dbo instanceof DBObject)
                        transformAttrs((DBObject)dbo, toToken);
            } else 
                if(value instanceof DBObject)
                    transformAttrs((DBObject)value, toToken);
        }
    }
    
    
    private String mapFieldToToken(String field) {
    ...
    // converts integer n into a base b string
     ...
     public String toBinaryString(int n) { return toString(n,  2); }
     public String toHexString(int n)    { return toString(n, 16); }
     ...
     public int fromBinaryString(String s) { return fromString(s,  2); }
     public int fromHexString(String s)    { return fromString(s, 16); }
